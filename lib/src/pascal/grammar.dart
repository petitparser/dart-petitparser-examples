...
class PascalGrammarDefinition extends GrammarDefinition {
  ...
  Parser statementCall() => seq2(
        ref0(identifier),
        seq3(ref1(token, '('), ref0(expression).plusSeparated(ref1(token, ',')),
                ref1(token, ')'))
            .optional(),
      );
  ...
  Parser typeRecord() => seq3(
        ref1(token, 'record'),
        ref0(fieldList),
        ref1(token, 'end'),
      );

  Parser typeFile() => seq2(
        ref1(token, 'file'),
        seq2(ref1(token, 'of'), ref0(type)).optional(),
      );
  ...
  Parser simpleType() => [
        seq3(ref1(token, '('), ref0(identifier).plusSeparated(ref1(token, ',')),
            ref1(token, ')')),
  ...
  Parser fieldListCase() => seq5(
      ...
      ref0(identifier),
      ref1(token, 'of'),
      seq5(ref0(constant).plusSeparated(ref1(token, ',')), ref1(token, ':'),
              ref1(token, '('), ref0(fieldList), ref1(token, ')'))
          .plusSeparated(ref1(token, ';')));
  ...
  Parser token(Object source) {
    if (source is String) {
      ...
      if (_isKeyword.accept(source)) {
        ...
        return token(source
            .toParser(caseInsensitive: true, message: message)
            .skip(after: word().not()));
      } else {
